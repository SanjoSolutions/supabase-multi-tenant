...
alter table "public"."tenants" add constraint "tenant_super_admin_id_fkey" FOREIGN KEY (super_admin_id) REFERENCES auth.users(id) ON UPDATE CASCADE ON DELETE CASCADE not valid;
...
create policy "Only creator of the tenant can give themself roles for a tenant"
on "public"."tenant_membership_roles"
as PERMISSIVE
for INSERT
to authenticated
with check (
  user_id = (SELECT auth.uid()) AND user_id = (SELECT super_admin_id FROM tenants WHERE tenants.id = tenant_id)
);
...
create policy "Only super admin of the tenant can add themself as member"
on "public"."tenant_memberships"
as PERMISSIVE
for INSERT
to authenticated
...
to authenticated
with check (
  super_admin_id = (SELECT auth.uid())
);
